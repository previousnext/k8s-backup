...
[![CircleCI](https://circleci.com/gh/previousnext/k8s-backup.svg?style=svg)](https://circleci.com/gh/previousnext/k8s-backup)

**Maintainer**: Nick Schuch

Automatically creates backup tasks for:
...
Next, using Git, clone this repository into $GOPATH/src/github.com/previousnext/k8s-backup. All the necessary dependencies are either vendored or automatically installed, so you just need to type `make test`. This will run the tests and compile the binary. If this exits with exit status 0, then everything is working!

```bash
$ cd "$GOPATH/src/github.com/previousnext/k8s-backup"
$ make test
```

To compile a development version of k8s-backup, run `make build`. This will build everything using gox and put binaries in the bin and $GOPATH/bin folders:
...
# Linux:
$ bin/k8s-backup_linux_amd64 --help

# OSX:
$ bin/k8s-backup_darwin_amd64 --help
```
...
$ docker-compose run --rm dev bin/k8s-backup_linux_amd64 --help
```

### Dependencies

k8s-backup stores its dependencies under `vendor/`, which [Go 1.6+ will automatically recognize and load](https://golang.org/cmd/go/#hdr-Vendor_Directories). We use [`dep`](https://github.com/golang/dep) to manage the vendored dependencies.
...
Release artifacts are pushed to the [github releases page](https://github.com/previousnext/k8s-backup/releases) when tagged
